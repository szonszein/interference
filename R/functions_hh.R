...
estimates_hierarchical <- function(estimator_data) {
  ...
  # causal effects
  estimator_data_group_ov <- estimator_data[, .(y_hat_gr_ov=mean(obs_outcome)), by=c('group', 'group_tr')]  
  estimator_data_pop_ov <- estimator_data_group_ov[, .(y_hat_ov=mean(y_hat_gr_ov)), by=c('group_tr')]

  estimator_data_group <- estimator_data[, .(y_hat_gr=mean(obs_outcome)), by=c('group', 'group_tr', 'indiv_tr')]
  estimator_data_pop <- estimator_data_group[, .(y_hat=mean(y_hat_gr)), by=c('group_tr', 'indiv_tr')]
  ...
  # variance estimators
  sample_var_group <- estimator_data[, .(var_hat_gr=var(obs_outcome), K_gr=.N), by=c('group', 'group_tr', 'indiv_tr')] # sigma^2_i
  sample_var_pop <- estimator_data_group[, .(var_hat=var(y_hat_gr)), by=c('group_tr', 'indiv_tr')] # sigma^2_g
  sample_var_pop <- merge(sample_var_pop, estimator_data[ , .(C=(.SD[unique(group), .N])), by=group_tr], by='group_tr')
  
  sample_var_pop_ov <- estimator_data_group_ov[, .(var_hat_ov=var(y_hat_gr_ov)), by=group_tr]
  sample_var_pop_ov <- merge(sample_var_pop_ov, estimator_data[ , .(C=(.SD[unique(group), .N])), by=group_tr], by='group_tr')
  
  var_direct_group <- sample_var_group[, .(var_hat_direct_gr=sum(var_hat_gr/K_gr, na.rm=TRUE)), by=c('group', 'group_tr')] # Var(CE_i^D)
  # 2nd component var hat CE
  second_var_direct <- var_direct_group[, .(sum_var_hat_direct=sum(var_hat_direct_gr), C=.N), by=group_tr] # sum Var(CE_i^D)
  
  direct_group <- estimator_data_group[, .(direct_hat_gr=(.SD[indiv_tr==1, y_hat_gr]-.SD[indiv_tr==0, y_hat_gr])), by=.(group, group_tr)]
  # 1st component var hat CE
  first_var_direct <- direct_group[, .(sample_var_hat_direct=var(direct_hat_gr)), by=group_tr] # sigma^2_D
...
estimands_hierarchical <- function(potential_outcomes, group, k) {
  ...
  estimand <- estimand[, .(direct_psi_group=mean(direct_psi), direct_phi_group=mean(direct_phi), indirect_group=mean(indirect),
                           total_group=mean(total), overall_group=mean(overall)), by=group]
  
  estimand <- estimand[, .(direct_psi=mean(direct_psi_group), direct_phi=mean(direct_phi_group), indirect=mean(indirect_group),
